...
def load_uri_fixture() -> dict:
    with (ROOT / "spec/fixtures/cite-contract.json").open() as fh:
        return json.load(fh)
...
def test_ambiguous_fuzzy_vanity_json_error() -> None:
    policy = Policy(
        ...
        vanity_aliases=[
            VanityAlias(from_="task://shortcuta", to="task://hop-top/cite/T-0001"),
            VanityAlias(from_="task://shortcutb", to="task://hop-top/cite/T-0002"),
        ],
    )
    ...
    assert exc.value.to_json() == {
        "input": "task://shortcut",
        "candidates": [
            {"from": "task://shortcuta", "to": "task://hop-top/cite/T-0001", "distance": 1},
            {"from": "task://shortcutb", "to": "task://hop-top/cite/T-0002", "distance": 1},
        ],
    }
...
def test_registry_parse_complete_and_types_parity() -> None:
    ...
    with pytest.raises(ValueError, match="unknown type"):
        registry.parse("task://hop-top/cite/T-0001")
    ...
    got = registry.parse("task://hop-top/cite/T-0001")
    assert got.namespace == "hop-top/cite"
