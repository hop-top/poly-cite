...
class Policy:
    ...
    def resolve_action(self, uri: URI) -> ResolvedAction:
        if uri is None:
            raise ValueError("cite: nil URI")
        if not uri.action:
            raise ValueError("cite: action is required")
        route = self.action_routes.get(uri.action)
        if route is None:
            raise ValueError(f"cite: unknown action {uri.action!r}")
        if not route.command:
            raise ValueError("cite: action route command is required")
    ...
    def resolve_vanity(self, input: str, options: ParseOptions) -> tuple[str, str]:
        ...
        for alias in self.vanity_aliases:
            if not alias.from_ or not alias.to:
                raise ValueError("cite: vanity alias from and to are required")
...
class AmbiguousVanityError(ValueError):
    ...
    def _message(self) -> str:
        ...
        return f"cite: ambiguous vanity alias {self.input!r}: {choices}"
...
def parse(input: str, policy: Policy | dict[str, object] | None = None, options: ParseOptions | dict[str, object] | None = None) -> URI:
    ...
    if input == "":
        raise ValueError("cite: empty input")

    parse_input, vanity = parsed_policy.resolve_vanity(input, parsed_options)
    parsed = urlparse(parse_input)
    if not parsed.scheme:
        raise ValueError("cite: scheme is required")
    if not parsed.netloc:
        raise ValueError("cite: namespace is required")
    ...
    if namespace_segments <= 0:
        raise ValueError("cite: namespace segment count must be positive")
    if len(segments) <= namespace_segments:
        raise ValueError("cite: id is required")
...
def _action_from_query(raw_query: str) -> str:
    ...
    if cmd or verb:
        if not cmd or not verb:
            raise ValueError("cite: cmd and verb must be provided together")
    ...
    if name:
        if not named_action:
            raise ValueError("cite: name and action must be provided together")
        candidates.append(f"{name}.{named_action}")

    if not candidates:
        return ""
    if any(candidate != candidates[0] for candidate in candidates[1:]):
        raise ValueError("cite: conflicting action query parameters")
    return candidates[0]
