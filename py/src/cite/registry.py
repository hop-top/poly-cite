...
class Registry:
    ...
    def register(self, reg: TypeRegistration) -> None:
        if not reg.name:
            raise ValueError("cite: registration name is required")
        if reg.name in self._types:
            raise ValueError(f"cite: type {reg.name!r} already registered")
    ...
    def parse(self, input: str) -> URI:
        ...
        if reg is None:
            raise ValueError(f"cite: unknown type {parsed.scheme!r}")
    ...
    def complete(self, type_name: str, prefix: str) -> list[str] | None:
        ...
        if reg is None:
            raise ValueError(f"cite: unknown type {type_name!r}")
