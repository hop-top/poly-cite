...
func loadContract(t *testing.T) contractFixture {
	...
	raw, err := os.ReadFile("../../spec/fixtures/cite-contract.json")
...
func TestParseWithPolicy_VanityExactAlias(t *testing.T) {
	...
	assert.Equal(t, "hop-top/cite", got.Namespace)
	assert.Equal(t, "T-0001", got.ID)
	assert.Equal(t, "task://hop-top/cite/T-0001", got.Canonical())
...
func TestParseWithPolicy_VanityPrefixAlias(t *testing.T) {
	...
	assert.Equal(t, "task://hop-top/cite/T-0001/child", got.Canonical())
