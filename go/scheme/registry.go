...
func (r *Registry) Register(reg TypeRegistration) error {
	if reg.Name == "" {
		return fmt.Errorf("cite: registration name is required")
	}
	...
	if _, exists := r.types[reg.Name]; exists {
		return fmt.Errorf("cite: type %q already registered", reg.Name)
	}
...
func (r *Registry) Parse(input string) (*URI, error) {
	...
	r.mu.RUnlock()
	if !exists {
		return nil, fmt.Errorf("cite: unknown type %q", parsed.Scheme)
	}
	if reg.Parser != nil {
...
func (r *Registry) Complete(ctx context.Context, typeName, prefix string) ([]string, error) {
	...
	if !exists {
		return nil, fmt.Errorf("cite: unknown type %q", typeName)
	}
