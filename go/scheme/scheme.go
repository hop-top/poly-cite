...
func (e AmbiguousVanityError) Error() string {
	...
	return fmt.Sprintf("cite: ambiguous vanity alias %q: %s", e.Input, strings.Join(parts, ", "))
}
...
func ParseWithPolicyOptions(input string, policy Policy, opts ...ParseOption) (*URI, error) {
	if input == "" {
		return nil, fmt.Errorf("cite: empty input")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("cite: invalid input: %w", err)
	}
	if parsed.Scheme == "" {
		return nil, fmt.Errorf("cite: scheme is required")
	}
	if parsed.Host == "" {
		return nil, fmt.Errorf("cite: namespace is required")
	}
	...
	for _, segment := range strings.Split(strings.TrimPrefix(parsed.EscapedPath(), "/"), "/") {
		if segment != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("cite: invalid path segment: %w", err)
			}
	...
	if namespaceSegments <= 0 {
		return nil, fmt.Errorf("cite: namespace segment count must be positive")
	}
	if len(segments) <= namespaceSegments {
		return nil, fmt.Errorf("cite: id is required")
	}
...
func actionFromQuery(rawQuery string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("cite: invalid query: %w", err)
	}
	...
	if cmd, verb := values.Get("cmd"), values.Get("verb"); cmd != "" || verb != "" {
		if cmd == "" || verb == "" {
			return "", fmt.Errorf("cite: cmd and verb must be provided together")
		}
		candidates = append(candidates, cmd+"."+verb)
	}
	if name, action := values.Get("name"), values.Get("action"); name != "" {
		if action == "" {
			return "", fmt.Errorf("cite: name and action must be provided together")
		}
	...
	for _, candidate := range candidates[1:] {
		if candidate != candidates[0] {
			return "", fmt.Errorf("cite: conflicting action query parameters")
		}
	}
...
func (p Policy) ResolveAction(uri *URI) (*ResolvedAction, error) {
	if uri == nil {
		return nil, fmt.Errorf("cite: nil URI")
	}
	if uri.Action == "" {
		return nil, fmt.Errorf("cite: action is required")
	}
	route, ok := p.ActionRoutes[uri.Action]
	if !ok {
		return nil, fmt.Errorf("cite: unknown action %q", uri.Action)
	}
	if route.Command == "" {
		return nil, fmt.Errorf("cite: action route command is required")
	}
...
func (p Policy) resolveVanity(input string, options ParseOptions) (parseInput string, vanity string, err error) {
	...
	for _, candidate := range p.VanityAliases {
		if candidate.From == "" || candidate.To == "" {
			return "", "", fmt.Errorf("cite: vanity alias from and to are required")
		}
		if strings.Contains(candidate.To, "://") {
			if _, parseErr := url.Parse(candidate.To); parseErr != nil {
				return "", "", fmt.Errorf("cite: invalid vanity target: %w", parseErr)
			}
		}
